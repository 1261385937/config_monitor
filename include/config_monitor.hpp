...
#define HAS_MEMBER(FUN)                                                                                              \
    ...
    template <typename T>                                                                                            \
    struct has_##FUN<T, std::enable_if_t<std::is_member_function_pointer_v<decltype(&T::FUN)>>> : std::true_type {}; \
    template <class T>                                                                                               \
...
private:
    std::unordered_map<std::string, std::unordered_set<std::string>> last_sub_path_;                // key is main path
    std::unordered_map<std::string, std::unordered_map<std::string, std::string>> sub_path_value_;  // key is main path

    std::mutex record_mtx_;
    std::unordered_map<std::string, std::unordered_map<watch_type, watch_cb>> record_;  // key is main path
public:
                ...
                if constexpr (std::is_same_v<ConfigType, zk::cppzk>) {
                    ...
                    for (const auto& r : record) {
                        for (const auto& type_cb : r.second) {
                            type_cb.first == watch_type::watch_path ? watch_path(r.first, type_cb.second)
    ...
    void watch_path(std::string_view path, watch_cb cb) {
        ...
        ConfigType::exists_path(path, [this, cb, p = std::string(path)](auto err, auto eve) {
            if (ConfigType::is_no_error(err) && (ConfigType::is_dummy_event(eve) || ConfigType::is_create_event(eve))) {
                ConfigType::get_path_value(p, [cb, this](auto err, std::optional<std::string>&& value) {
    ...
    void watch_sub_path(std::string_view path, watch_cb cb) {
        ...
        ConfigType::exists_path(path, [this, cb, main_path = std::string(path)](auto err, auto eve) {
            ...
            ConfigType::get_sub_path(
                main_path, [this, cb, main_path](auto e, auto, std::vector<std::string>&& sub_paths) {
    ...
    void create_path(std::string_view path, create_cb cb, bool is_ephemeral = false, bool is_sequential = false,
        ...
        if (sp_path.size() == 1) {
            ConfigType::create_path(path, value, create_mode, [cb, this](auto e, std::string&& path) {
                if (cb) {
        ...
        ConfigType::create_path(sp_path.back(), value, create_mode, [cb, this](auto e, std::string&& path) {
            if (cb) {
                cb(ConfigType::is_no_error(e) ? true : false, path);
            }
        });
    ...
    void create_path_sync(std::string_view path, bool is_ephemeral = false, bool is_sequential = false,
                          std::string_view value = "") {
        auto r = ConfigType::exists_path_sync(path);
        if (ConfigType::is_no_error(r) && !is_sequential) {
            return;
