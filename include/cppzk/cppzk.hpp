...
public:
	...
	zk_error create_path_sync(std::string_view path, std::string_view value, zk_create_mode mode,
							  std::string& new_path, int64_t ttl = -1,
							  zk_acl acl = zk_acl::zk_open_acl_unsafe) {
		std::string newpath;
		newpath.resize(4096);
		auto r = zoo_create2_ttl(zh_, path.data(), value.data(), (int)(value.length()),
								 &acl_mapping[acl], (int)mode, ttl, newpath.data(),
								 (int)newpath.length(), nullptr);
