...
namespace zk {
inline std::unordered_map<zk_acl, ACL_vector> acl_mapping{{zk_acl::zk_open_acl_unsafe, ZOO_OPEN_ACL_UNSAFE},
...
struct exists_userdata : user_data {
    ...
    exists_userdata(watcher_fn f, stat_completion_t c, exists_callback callbback, cppzk* ptr, std::string_view p)
...
struct wget_userdata : user_data {
    ...
    wget_userdata(watcher_fn f, data_completion_t c, get_callback callbback, cppzk* ptr, std::string_view p)
        : wfn(f), completion(c), cb(std::move(callbback)), self(ptr), path(p) {
    }
};
struct get_children_userdata : user_data {
    ...
    get_children_userdata(watcher_fn f, strings_stat_completion_t c, get_children_callback callbback, cppzk* ptr,
                          std::string_view p)
