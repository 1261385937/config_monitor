...
public:
    ...
    void TearDown() override {
        ...
        auto err = cm::config_monitor<zk::cppzk>::instance().remove_watches(
            async_test_path, cm::watch_type::watch_path);
        ...
        err = cm::config_monitor<zk::cppzk>::instance().remove_watches(
            async_main_path, cm::watch_type::watch_sub_path);
...
TEST_P(cppzk_test, create_path_with_value) {
    std::string multilayer_path = test_path + "/1/2/3/4/5";
    auto [ec, path] = cm::config_monitor<zk::cppzk>::instance().create_path(multilayer_path, "333");
    EXPECT_EQ(ec.value(), 0);
    EXPECT_EQ(path, multilayer_path);

    auto [e, path_value] = cm::config_monitor<zk::cppzk>::instance().watch_path(multilayer_path);
    EXPECT_EQ(e.value(), 0);
    EXPECT_EQ(path_value, "333");
};

TEST_P(cppzk_test, create_path_without_value) {
    std::string multilayer_path = test_path + "/1/2/3/4/5";
    auto [ec, path] = cm::config_monitor<zk::cppzk>::instance().create_path(multilayer_path);
    EXPECT_EQ(ec.value(), 0);
    EXPECT_EQ(path, multilayer_path);

    auto [e, path_value] = cm::config_monitor<zk::cppzk>::instance().watch_path(multilayer_path);
    EXPECT_EQ(e.value(), 0);
    EXPECT_EQ(path_value.has_value(), false);
};


TEST_P(cppzk_test, create_exist_path) {
    std::string multilayer_path = test_path + "/1/2/3/4/5";
    auto [ec0, path0] = cm::config_monitor<zk::cppzk>::instance().create_path(multilayer_path);
    EXPECT_EQ(ec0.value(), 0);
    EXPECT_EQ(path0, multilayer_path);
    auto [ec1, path1] = cm::config_monitor<zk::cppzk>::instance().create_path(multilayer_path);
...
TEST_P(cppzk_test, async_create_path_with_value) {
    ...
    cm::config_monitor<zk::cppzk>::instance().async_create_path(multilayer_path,
...
TEST_P(cppzk_test, async_create_path_without_value) {
    std::string multilayer_path = async_test_path + "/1/2/3/4/5";
    std::promise<std::pair<std::error_code, std::string>> pro;
    cm::config_monitor<zk::cppzk>::instance().async_create_path(multilayer_path,
        [&pro](const std::error_code& ec, std::string&& path) {
...
TEST_P(cppzk_test, async_create_exist_path) {
    std::string multilayer_path = async_test_path + "/1/2/3/4/5";
    cm::config_monitor<zk::cppzk>::instance().create_path(multilayer_path);
    std::promise<std::pair<std::error_code, std::string>> pro;
...
TEST_P(cppzk_test, set_path_value) {
    ...
    auto [e, path_value] = cm::config_monitor<zk::cppzk>::instance().watch_path(test_path);
    EXPECT_EQ(e.value(), 0);
...
TEST_P(cppzk_test, async_set_path_value) {
    ...
    auto [e, path_value] = cm::config_monitor<zk::cppzk>::instance().watch_path(async_test_path);
    EXPECT_EQ(e.value(), 0);
...
TEST_P(cppzk_test, async_watch_sub_path) {
    ...
    cm::config_monitor<zk::cppzk>::instance().async_watch_sub_path(async_main_path,
...
TEST_P(cppzk_test, async_watch_sub_path_not_exist_then_create) {
...
TEST_P(cppzk_test, async_watch_sub_path_change_value) {
...
TEST_P(cppzk_test, async_watch_sub_path_delete_path) {
